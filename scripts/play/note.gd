...
var NoteTex = preload("res://media/textures/note.png")
var MoveTex = preload("res://media/textures/note2.png")
var SpikeTex = preload("res://media/textures/note3.png")
func _ready() -> void:
...
func _process(delta: float) -> void:
	if NoteSprite.modulate.a < 1:
		NoteSprite.modulate.a += delta / 1.2 * Settings.note_speed
		ArrowSprite.modulate.a += delta / 1.2 * Settings.note_speed
		if NoteSprite.modulate.a > 1:
	...
	position.z = Game.panelSize -((time - Game.currentTime) * Settings.note_speed / 1000)
