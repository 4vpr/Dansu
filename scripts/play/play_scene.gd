...
func _ready() -> void:
	...
	Engine.max_fps = Settings.game_fps
	Engine.physics_ticks_per_second = Settings.tick_rate
...
func _physics_process(delta:float) -> void:
	if paused:
		return
	if song_playing:
		Game.currentTime = ($SongPlayer.get_playback_position()
...
func write_judge(j: int,note):
	...
	if j != 0:
		...
		sfx_pool.play_sound(preload("res://media/sfx/drum.wav"))
