	...
	for file_path in files:
		...
		if file.to_lower().ends_with(".mp3"):
			...
			if src_file:
				...
				src_file.close()
				var dst_file = FileAccess.open(folder_path.path_join("song.mp3"), FileAccess.WRITE)
				if dst_file:
			...
			if json:
				json.store_string("{}")
				json.close()
			Game.select_folder = _add_beatmap_set(folder_path)
			_refresh()
func _add_new_difficulty():
	var path = Game.select_map.json_path
	...
	if file:
		...
		var target_file = FileAccess.open(Game.select_map.folder_path.path_join(json_name) , FileAccess.WRITE)
		if target_file:
			var json_string = JSON.stringify(json_data, "\t")
			target_file.store_string(json_string)
			target_file.close()
		var new_beatmap = Beatmap.new()
		new_beatmap.load_from_json(Game.select_map.folder_path.path_join(json_name))
		new_beatmap.folder_path = Game.select_folder.folder_path
		for child in $MapPanel/VBoxContainer.get_children():
			if child.beatmap_set == Game.select_folder:
				child.beatmap_set.beatmaps.append(new_beatmap)
				Game.select_map = new_beatmap
				child.reload_diff()
