...
func copy(selection: ChartEditorSelection) -> bool:
	...
	var next_data: Array[Dictionary] = []
	var earliest := 2147483647
	for i in range(owners.size()):
		var rail := owners[i]
		var item: Dictionary = captured[i]
		if not whole_rail:
			var notes: Array = []
		...
		for point: Dictionary in item.points:
			start = mini(start, point.time)
			finish = maxi(finish, point.time)
		for note: Dictionary in item.notes:
			start = mini(start, note.time)
			finish = maxi(finish, note.time + maxi(note.length, 0))
		item["start"] = start
		item["finish"] = finish
		item["start_x"] = rail._get_rail_x_at_time(start)
		item["end_x"] = rail._get_rail_x_at_time(finish)
	...
	data.sort_custom(func(a: Dictionary, b: Dictionary) -> bool:
		return a.id < b.id if is_equal_approx(a.start_x, b.start_x) else a.start_x < b.start_x
	)
	return true
...
func paste(editor: ChartEditor) -> bool:
	...
	var pasted_points: Array[Dictionary] = []
	for i in range(data.size()):
		...
		for point: Dictionary in item.points:
			...
			pasted_points.append({"rail": target, "point": placed})
		for note_data: Dictionary in item.notes:
			var note := Note.new()
	...
	if not pasted_notes.is_empty():
		...
		var primary: Dictionary = pasted_points[0]
		editor.selection.select_point(primary.rail, primary.rail.points.find(primary.point))
	for entry: Dictionary in pasted_points:
		editor.selection.selected_points[entry.point] = entry.rail
